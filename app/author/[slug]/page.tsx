...
export default async function AuthorPage({ params }: AuthorPageProps) {
    ...
    if (!author) {
        return (
            <div className="min-h-screen bg-background-light text-neutral-dark flex flex-col items-center justify-center p-4">
            ...
            <main className="max-w-7xl mx-auto px-4 py-8 md:py-12 text-left">
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row items-center md:items-start gap-8 md:gap-12">
                        ...
                        <div className="flex-1 text-center md:text-left">
                            ...
                            <div className="flex flex-wrap items-center justify-center md:justify-start gap-4">
                                {author.twitter && (
                                    <a href={author.twitter} target="_blank" rel="noopener noreferrer" className="p-3 rounded-xl bg-gray-50 hover:bg-primary text-neutral-dark/40 hover:text-white transition-all border border-black/5 shadow-sm">
                                        <Twitter size={20} />
                                    </a>
                                )}
                                {author.linkedin && (
                                    <a href={author.linkedin} target="_blank" rel="noopener noreferrer" className="p-3 rounded-xl bg-gray-50 hover:bg-primary text-neutral-dark/40 hover:text-white transition-all border border-black/5 shadow-sm">
                                        <Linkedin size={20} />
                                    </a>
                                )}
                                {author.facebook && (
                                    <a href={author.facebook} target="_blank" rel="noopener noreferrer" className="p-3 rounded-xl bg-gray-50 hover:bg-primary text-neutral-dark/40 hover:text-white transition-all border border-black/5 shadow-sm">
                                        <Facebook size={20} />
                                    </a>
                                )}
                                {author.instagram && (
                                    <a href={author.instagram} target="_blank" rel="noopener noreferrer" className="p-3 rounded-xl bg-gray-50 hover:bg-primary text-neutral-dark/40 hover:text-white transition-all border border-black/5 shadow-sm">
                                        <Instagram size={20} />
                                    </a>
                                )}
                                {author.youtube && (
                                    <a href={author.youtube} target="_blank" rel="noopener noreferrer" className="p-3 rounded-xl bg-gray-50 hover:bg-primary text-neutral-dark/40 hover:text-white transition-all border border-black/5 shadow-sm">
                                        <Youtube size={20} />
                                    </a>
                                )}
                                {author.website && (
                                    <a href={author.website} target="_blank" rel="noopener noreferrer" className="p-3 rounded-xl bg-gray-50 hover:bg-primary text-neutral-dark/40 hover:text-white transition-all border border-black/5 shadow-sm">
                                        <Globe size={20} />
                                    </a>
                                )}
                            </div>
                ...
                <div>
                    <div className="flex items-center gap-4 mb-8">
                        <h2 className="text-2xl md:text-3xl font-bold text-neutral-dark">Articles by {author.first_name}</h2>
                        <div className="h-px flex-1 bg-black/5"></div>
                        <span className="text-neutral-dark/30 text-sm font-display uppercase tracking-widest">{author.posts?.length || 0} Posts</span>
                    </div>

                    {author.posts && author.posts.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {author.posts.map((post) => (
                                <Link key={post.id} href={`/post/${post.id}/${post.slug}`} className="group bg-white rounded-2xl overflow-hidden border border-black/5 hover:border-primary/20 transition-all shadow-soft hover:shadow-medium">
                        ...
                        <div className="text-center py-20 bg-white rounded-3xl border border-black/5 shadow-soft">
                            <p className="text-neutral-dark/40">No articles published by this author yet.</p>
                        </div>
                    )}
                </div>
            </main>
