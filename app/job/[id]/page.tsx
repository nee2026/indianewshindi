...
import { ChevronRight, MapPin, Briefcase, IndianRupee, Clock, Building, GraduationCap, Users, Navigation } from "lucide-react";
import { useSearchParams } from "next/navigation";
import { fetchJobDetails } from "@/services/api";
import { HirexJob } from "@/types";
import { formatDate, toTitleCase, generateJobWhatsAppLink } from "@/lib/utils";
...
export default function JobDetailPage({ params }: JobPageProps) {
    ...
    const [job, setJob] = useState<HirexJob | null>(null);
    ...
    useEffect(() => {
        async function loadData() {
            setLoading(true);
            try {
                const data = await fetchJobDetails(id, pageNum);
                setJob(data);
            } catch (error) {
    ...
    if (!job) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center bg-background-light dark:bg-neutral-dark p-4">
                <div className="text-center py-20 bg-white dark:bg-neutral-800 rounded-3xl border border-black/5 dark:border-white/5 shadow-soft max-w-2xl w-full">
                    <Briefcase className="mx-auto h-16 w-16 text-neutral-300 dark:text-neutral-600 mb-6" />
                    <h1 className="text-3xl font-bold text-neutral-dark dark:text-white mb-4">Job Not Found</h1>
                    <p className="text-neutral-500 mb-8">The job you are looking for does not exist or has been removed.</p>
                    <Link href="/category/job" className="inline-flex items-center gap-2 bg-primary text-white px-6 py-3 rounded-xl font-bold hover:bg-primary/90 transition-colors">
                        Browse All Jobs
                    </Link>
                </div>
            </div>
